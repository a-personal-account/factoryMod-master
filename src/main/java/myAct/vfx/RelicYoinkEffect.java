...
public class RelicYoinkEffect extends AbstractGameEffect {
    ...
    public RelicYoinkEffect(AbstractRelic ar, float x, float y) {
        this.ar = ar;
        this.endX = x;
        this.endY = y;
    ...
    public void update() {
        if (frames < flighttime) {
            this.currentX = this.startX + sigmoid(endX - this.startX, 6F / flighttime, frames);
            this.currentY = this.startY + sigmoid(endY - this.startY, 6F / flighttime, frames++);
        } else if (opacity > 0) {
